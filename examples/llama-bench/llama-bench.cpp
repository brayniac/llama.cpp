...
static void print_usage(int /* argc */, char ** argv) {
    ...
    printf("  --mlock <0|1>                             (default: %s)\n", join(cmd_params_defaults.use_mlock, ",").c_str());
...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            auto p = string_split<bool>(argv[i], split_delim);
            params.use_mlock.insert(params.use_mlock.end(), p.begin(), p.end());
    ...
    if (params.use_mlock.empty())    { params.use_mlock = cmd_params_defaults.use_mlock; }
...
static std::vector<cmd_params_instance> get_cmd_params_instances(const cmd_params & params) {
    ...
    for (const auto & mlock : params.use_mlock)
...
struct test {
    ...
    static const std::vector<std::string> & get_fields() {
        static const std::vector<std::string> fields = {
            ...
            "tensor_split", "use_mmap", "use_mlock", "embeddings",
    ...
    static field_type get_field_type(const std::string & field) {
        ...
        if (field == "cuda" || field == "vulkan" || field == "kompute" || field == "metal" ||
            ...
            field == "cpu_strict" ||
            field == "flash_attn" || field == "use_mmap" || field == "use_mlock" || field == "embeddings") {
            return BOOL;
        }
    ...
    std::vector<std::string> get_values() const {
        ...
        std::vector<std::string> values = {
            ...
            tensor_split_str, std::to_string(use_mlock), std::to_string(embeddings),
...
struct markdown_printer : public printer {
    ...
    void print_header(const cmd_params & params) override {
        ...
        if (params.use_mlock.size() > 1 || params.use_mlock != cmd_params_defaults.use_mlock) {
            fields.emplace_back("use_mlock");
        }
